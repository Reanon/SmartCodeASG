...
def get_standard_json_old(contract_path, filename):
    ...
    # 读取文件内容
    content = ''
    with open(os.path.join(ROOT_DIR, filepath), 'r') as f:
        content = f.read()
    # 标准 json 输入
    standard_json = '''
    {
...
def get_one_file():
    ...
    # 多个合约
    filename = "0x96edbe868531bd23a6c05e9d0c424ea64fb1b78b.sol"
    filepath_input = {contract_path + filename: get_standard_json_old(contract_path, filename)}
...
def get_one_test_file():
    ...
    filename = "call.sol"
    file_input = {contract_path + filename: get_standard_json_old(contract_path, filename)}
    return file_input
