...
class GMNLayer(MessagePassing, ABC):
    ...
    def propagate_match(self, edge_index, size=None, **kwargs):
        ...
        out_attn = torch.scatter(self.aggr, out_attn, edge_index[i], dim_size=size[i], reduce='add')
...
class GMNNet(torch.nn.Module):
    def __init__(self, vocablen, embedding_dim, num_layers, device):
        ...
        self.gmn_layer = GMNLayer(embedding_dim, embedding_dim, self.device)
